    ...
    function EventCalendar(element, options) {
        ...
        var _changeCalendarMonth = function(show) {
            ...
            for (dayCount = 1; dayCount <= daysInMonth; dayCount += 1) {
				if(currant_day == dayCount && currant_month == dateToShow.getMonth() && currant_year == dateToShow.getFullYear() ){
					var class_current = " current_date";
				}
				else{
					var class_current = "";
				}
                calendarCells.push('<li id="dayList_' + dayCount + '" rel="' + dayCount + '" class="eventsCalendar-day'+class_current+'"><a href="#">' + dayCount + '</a></li>');
            }
