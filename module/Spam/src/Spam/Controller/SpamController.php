...
{     
	...
	function SpamsAction() {	
		...
		try {				
			...
			$spam_problem_id = $post->get('problem_id');
	...
	public function SpamsProblemsAction()
	{
		#db connectivity
		$request = $this->getRequest();
		...
		$ProblemData = $this->ProblemTable->fetchAllProblems();
		...
		$viewModel = new ViewModel(array('form' => $form,'ProblemData'=>$ProblemData , 'GroupId'=>$GroupId, 'SubGroupId'=>$SubGroupId, 'GroupReferId'=>$GroupReferId, 'SystemTypeId'=>$SystemTypeId));
		$viewModel->setTerminal($request->isXmlHttpRequest());
