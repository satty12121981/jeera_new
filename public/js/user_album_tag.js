jQuery(document).ready(function()
{
	...
	$('#done-tag-'+id).hide();
	$('#add-user-tag-'+id).show();
	});
	
	$(document).on("click",".add-user-tag",function(event){	 
	var valu = $(this).attr("id");
	var id = valu.replace('add-user-tag-','');
	$('#data_id').val(id);
	$('#album-img-'+id).addClass('tag-img');
	$('#add-user-tag-'+id).hide();
	$('#done-tag-'+id).show();
	});
	 ...
	 $(document).on("enterKey","#tagname",function(event){	 
	  ...
	  $.ajax({
		 ...
		 success:function(result){ 
			if(result){
			//alert (1);
			var display_tag = '<span class="album_tags" id="list_tag_'+result+'_'+tag_id+'"><a class="view_tag" href="#" id="view_tag_'+result+'|'+tag_id+'">'+name+'</a> (<a id="del_user_tag_'+result+'_'+tag_id+'" class="remove_user_tag" style="cursor:pointer;">X</a>)</span>';
	...
	$(document).on("keyup","#tagname",function(event){		
		...
		var path = baseurl+"/album/users/"+profile_name;
		$(this).autocomplete({
			source: path,
			minLength: 1,
...
$(".view_tag").hover(function(event){
  ...
  $(document).on("click",".remove_user_tag",function(event){ 	
      
	   var valu =  $(this).attr('id');
	   var id = valu.replace('del_user_tag_','');
	   var res = id.split('_');
	   var tag_id = res[0];
	   var user_id = res[1];
	   $.ajax({
