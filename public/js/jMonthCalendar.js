...
(function($) {
	...
	jQuery.J.DrawCalendar = function(dateIn){
		...
		var prevmonthcnt = d.getMonth()==0?11:d.getMonth()-1;
		var prevMonthname = d.getMonth() == 0 ? new Date(d.getFullYear()-1, 11, 1) : new Date(d.getFullYear(), d.getMonth()-1, 1);
		var prevMonthNameLink = jQuery('<div class="MonthNamePrev"><a href="" class="link-prev">'+ defaults.locale.months[prevmonthcnt]  +'</a></div>').click(function() {
			jQuery.J.ChangeMonth(prevMonthname);
			return false;
		});
		
		//Build up the Header first,  Navigation
		var navRow = jQuery('<tr><td colspan="'+maxDays+'" align="center"><div class="FormHeader MonthNavigation"></div></td></tr>').css({ "height" : defaults.navHeight });
		var monthNavHead = jQuery('.MonthNavigation', navRow);
		
		monthNavHead.append(prevMLink);
		monthNavHead.append(prevMonthNameLink);
		

		monthNavHead.append(jQuery('<div class="MonthName"></div>').append(defaults.locale.months[d.getMonth()] + " " + d.getFullYear()));
		var nextMonthcnt = d.getMonth() == 11 ? 0 : d.getMonth()+1; 
		var nextMonthName = d.getMonth() == 11 ? new Date(d.getFullYear()+1, 0, 1) : new Date(d.getFullYear(), d.getMonth()+1, 1); 
		var nextMonthNameLink = jQuery('<div class="MonthNameNext"><a href="" class="link-next">'+ defaults.locale.months[nextMonthcnt] +'</a></div>').click(function() {
			jQuery.J.ChangeMonth(nextMonthName);
			return false;
		});
		
		
		monthNavHead.append(nextMonthNameLink);
	...
	//	if(defaults.navLinks.enableNextYear) { monthNavHead.append(nextYLink); }
		...
		var maxDays = Date.getDaysInMonth(d.getFullYear(), d.getMonth());
		var curDay1 = defaults.firstDayOfWeek - d.getDay();
		for(i=0;i<maxDays;i++){
		...
		for (var i=firstday; i<maxDays+firstday; i++) {
			var weekday = i%7;
			var wordday = defaults.locale.daysMin[weekday];
		...
		do {
			...
			for (var i=0; i<maxDays; i++) {
				...
				//DateBox Events
				var dateLink = jQuery('<div class="DateLabel"><a href="javascript:void(0)" id="'+_currentDate.toString("M/d/yyyy")+'">' + _currentDate.getDate() + '</a></div>').click(function(e) {
		...
		var cal = jQuery('<table class="MonthlyCalendar" cellpadding="0" tablespacing="0" width="100%"></table>').append(headRow, tBody);
